    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="inline-flex items-center">
                <svg class=" text-gray-500 dark:text-neutral-500" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                    <path d="M19 21v-2a4 4 0 0 0-4-4H9a4 4 0 0 0-4 4v2"></path>
                    <circle cx="12" cy="7" r="4"></circle>
                </svg>
                <input type="text" name="code_mapel" value="{{$mapel->code_mapel}}" class="@error('code_mapel') is-invalid @enderror ml-2 py-3 px-4 block w-full bg-gray-100 border-transparent rounded-lg text-sm focus:border-blue-500 focus:ring-blue-500 dark:bg-neutral-700 dark:text-neutral-400 dark:placeholder-neutral-500 dark:focus:ring-neutral-600" placeholder="Kode Mapel">
                @error('code_mapel')
                <strong class="text-sm text-red-600 ml-4">{{ $message }}</strong>
                @enderror
                <input type="text" name="nama_mapel" value="{{$mapel->nama_mapel}}" class="@error('nama_mapel') is-invalid @enderror ml-2 py-3 px-4 block w-full bg-gray-100 border-transparent rounded-lg text-sm focus:border-blue-500 focus:ring-blue-500 dark:bg-neutral-700 dark:text-neutral-400 dark:placeholder-neutral-500 dark:focus:ring-neutral-600" placeholder="Nama Mapel">
                @error('nama_mapel')
                <strong class="text-sm text-red-600 ml-4">{{ $message }}</strong>
                @enderror
                <input type="number" name="jumlah_jp" value="{{$mapel->jumlah_jp}}" class="@error('jumlah_jp') is-invalid @enderror ml-2 py-3 px-4 block w-full bg-gray-100 border-transparent rounded-lg text-sm focus:border-blue-500 focus:ring-blue-500 dark:bg-neutral-700 dark:text-neutral-400 dark:placeholder-neutral-500 dark:focus:ring-neutral-600" placeholder="Jumlah JP">
                @error('jumlah_jp')
                <strong class="text-sm text-red-600 ml-4">{{ $message }}</strong>
                @enderror
                <input type="text" name="nama_guru" value="{{$mapel->nama_guru}}" class="@error('nama_guru') is-invalid @enderror ml-2 py-3 px-4 block w-full bg-gray-100 border-transparent rounded-lg text-sm focus:border-blue-500 focus:ring-blue-500 dark:bg-neutral-700 dark:text-neutral-400 dark:placeholder-neutral-500 dark:focus:ring-neutral-600" placeholder="Nama Guru">
                @error('nama_guru')
                <strong class="text-sm text-red-600 ml-4">{{ $message }}</strong>
                @enderror
