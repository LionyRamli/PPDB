<x-app-layout>
                    ...
                    <table class="min-w-full bg-white dark:bg-neutral-900 rounded-lg shadow-lg overflow-hidden">
                        <thead class="bg-gray-200 dark:bg-neutral-800 text-gray-600 dark:text-gray-300">
                            <tr>
                                ...
                                <th class="py-3 px-5 text-left">Materials Code</th>
                                <th class="py-3 px-5 text-left">Materials Name</th>
                                <th class="py-3 px-5 text-center">Accumulated Learning Hours</th>
                                <th class="py-3 px-5 text-left">Teacher Name</th>
                            ...
                            @if(count($mapels) > 0)
                                @foreach($mapels as $key => $mapel)
                                    <tr class="hover:bg-gray-100 dark:hover:bg-neutral-800 transition duration-150">
                                        ...
                                        <td class="py-3 px-5">{{$mapel->code_mapel}}</td>
                                        <td class="py-3 px-5">{{$mapel->nama_mapel}}</td>
                                        <td class="py-3 px-5">{{$mapel->jumlah_jp}}</td>
                                        <td class="py-3 px-5">{{$mapel->nama_guru}}</td>
                                        ...
                                        <td class="py-3 px-5 text-center">
                                            <button onclick="openDeleteModal({{$mapel->id}})" class="p-1 text-sm font-semibold rounded-md border border-red-600 bg-red-600 text-white hover:bg-red-700 transition duration-150">Delete</button>
                                        </td>
                                    </tr>
                                @endforeach
                            @else
    ...
    <script>
        let deleteMapelId = null;

        function openDeleteModal(mapelId) {
            deleteMapelId = mapelId;
        ...
        function confirmDelete() {
            if (deleteMapelId) {
                ...
                form.action = `/mapel/${deleteMapelId}/destroy`;
                form.submit();
            }
        }
    </script>
