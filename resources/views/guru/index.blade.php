<x-app-layout>
                    ...
                    <table class="min-w-full bg-white dark:bg-neutral-900 rounded-lg shadow-lg overflow-hidden">
                        <thead class="bg-gray-200 dark:bg-neutral-800 text-gray-600 dark:text-gray-300">
                            <tr>
                                ...
                                <th class="py-3 px-5 text-left">Teacher Code</th>
                                <th class="py-3 px-5 text-left">Teacher Name</th>
                                <th class="py-3 px-5 text-center">Phone Number</th>
                            ...
                            @if(count($gurus) > 0)
                                @foreach($gurus as $key => $guru)
                                    <tr class="hover:bg-gray-100 dark:hover:bg-neutral-800 transition duration-150">
                                        ...
                                        <td class="py-3 px-5">{{$guru->code_guru}}</td>
                                        <td class="py-3 px-5">{{$guru->nama_guru}}</td>
                                        <td class="py-3 px-5">{{$guru->no_hp}}</td>
                                        ...
                                        <td class="py-3 px-5 text-center">
                                            <button onclick="openDeleteModal({{$guru->id}})" class="p-1 text-sm font-semibold rounded-md border border-red-600 bg-red-600 text-white hover:bg-red-700 transition duration-150">Delete</button>
                                        </td>
                                    </tr>
                                @endforeach
                            @else
    ...
    <script>
        ...
        function openDeleteModal(guruId) {
            deleteGuruId = guruId;
        ...
        function confirmDelete() {
            if (deleteGuruId) {
                ...
                form.action = `/guru/${deleteGuruId}/destroy`;
                form.submit();
            }
        }
    </script>
